...
void e8253_init (e8253_t *pit)
{
	...
	//my addition
	pit->timerZero=0; pit->timerTwo=0;
}
...
void e8253_set_uint8 (e8253_t *pit, unsigned long addr, unsigned char val)
{
	...
	if (addr < 3) {
		...
		if(addr==0) pit->timerZero=0;
		if(addr==2) pit->timerTwo=0;
	}
	else if (addr == 3) {
		cnt_i = (val >> 6) & 3;
...
void e8253_clock (e8253_t *pit, unsigned n)
{
	...
	if(pit->counter[0].counting && pit->counter[0].val==0)
		pit->timerZero=1;
...
{
	return (pit->timerTwo<<1)|(pit->timerZero);
}

void e8253_set_status(e8253_t *pit, unsigned char value)
{
	pit->timerZero &= (value&1)==1;
	pit->timerTwo &= (value&2)==1;
}
