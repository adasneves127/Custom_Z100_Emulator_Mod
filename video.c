		...
		if((actual_scan_line&0xf) < 9) {
			...
			for(int charXpos = 0; charXpos < VWIDTH/8; charXpos++) {
				...
				for(int bit = 0; bit < 8; bit++) {
					...
					int blue = (v->vram[(raw_addr+(v->addressLatch*128*16))&0xffff]>>bit)&1;
					if(!v->blueenabled) blue=0;
					int red = (v->vram[((raw_addr+(v->addressLatch*128*16))&0xffff)+0x10000]>>bit)&1;
					if(!v->redenabled) red=0;
					int green = (v->vram[((raw_addr+(v->addressLatch*128*16))&0xffff)+0x20000]>>bit)&1;
