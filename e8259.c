...
{
	// assign PIC label - "MASTER" or "SLAVE"
	pic->label=lbl;
...
unsigned char e8259_inta (e8259_t *master_pic, e8259_t *slave_pic)
{
	...
	// clear the master interrupt INT pin
	e8259_set_int (master_pic, 0);
	...
	if (irrp >= 16) {
		...
		return (master_pic->base + 7);
	}

	// irrp = irrp & 0b0000 0111
	// the priority will be 0-7
	irrp &= 7;
	// get the IRR bit - the interrupt pin (IR0-IR7)
	irrb = 1 << irrp;

	// set the IRR register to IRR & ~irrb
	/* example: if irrb = 3 (0b0000 0100), we are clearing that bit and keeping
	the rest of the bits that have been set from other interrupts */
	master_pic->irr &= ~irrb;

	if (master_pic->icw[3] & E8259_ICW4_AEOI) {
		if (master_pic->rot_on_aeoi) {
			master_pic->priority = (irrp + 1) & 7;
		}
	}
	else {
		master_pic->isr |= irrb;
